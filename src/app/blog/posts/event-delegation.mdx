...
React's custom event system started as an effort to normalize browser behaviour. By managing events internally, React controls how bubbling works across all events. In fact, as we mentioned before, even if you listen at the parent level, you'll find that the bubbled event is still a React synthetic event (caveat: When using React event handlers)
...
But there's an interesting solution we already discussed: event bubbling. Many events naturally bubble. When React detects that a user has attached an event handler (like onClick), it adds an onClick to the root React element. Once the event bubbles up to the root, React identifies the event and its origin, then passes the same information to the original event handler (our onClick listener). This way, React has much fewer event listerners on the DOM but gets to listen in on all of them.

Try to guess the output of document event click in the following example:
...
It prints the root-level element's console log, even though the React event listener has `event.stopPropagation()` applied. However, it does not print the document console log if you're using React 17 or above. Why is that?

As mentioned, React attaches event listeners to the root element. The user has added `stopPropagation()` to the React event listener, but the event must travel to the root element for React to recognize that a click event has occurred. By the time it reaches the root element, the click event there has already finished executing. However, the document event has not executed, so React cancels it out.

Note that this is because we mixed event patterns, the root element event listener is in JavaScript and not React.
